...
$(document).ready(function () {
    ...
    switch (pageName) {
        ...
        case 'provider_add':
            ...
            $('.upload_company_brand').on('change', uploadSingleImage);
        ...
        success: function (data, textStatus, jqXHR) {
            if (typeof data.error === 'undefined') {
                if (data['status'] == true) {
                    ...
                    $("#" + str + "_src").val(JSON.stringify([data['originfile'], url]));
                }
            }
            else {
                // Handle errors here
...
function uploadImageAndInsertTag(event) {
            ...
            if (typeof data.error === 'undefined') {
                if (data['status'] == true) {
                    ...
                    content_html +=  '<div class="company_brand" style="float: left;">' +
                                    ...
                                    '           value=\'' + JSON.stringify([data['originfile'], url]) + '\'>' +
    ...
    $.ajax({
        type: 'post',
        url: baseURL + 'provider_controller/item_listing',
        dataType: 'json',
...
function deleteItem() {
    ...
    $.ajax({
        url: baseURL + "provider_controller/deleteItemFromDB",
        type: "POST",
        data: {'itemInfo': itemInfo},
        success: function (result) {
            console.log(result);
