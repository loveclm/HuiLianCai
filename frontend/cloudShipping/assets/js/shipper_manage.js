...
$(document).ready(function () {
    ...
    loadDatafromStorage();
    resize_main_page();
    sendLoginRequest(getPhoneNumber(), getSessionPassword(), 1);
});
...
function loadDatafromStorage() {
    ...
    data.phone_num = getUserInfo().userid;
...
function display_advertise_images() {
    ...
    if (data.advertise_imgs != undefined) {
        ...
        carousel_content_html += '<img class="user-head-img0" src="';
        carousel_content_html += getImageURL(shipperData.user_image) + '">';
        carousel_content_html += '<h5><span class="shop-detail-head" style="bottom:20px;">';
        carousel_content_html += shipperData.user_name ;
