...
function showShippingOrderList() {
    ...
    loadDatafromStorage();
    selectMenu((parseInt(sessionStorage.getItem('cur_menu_index')) - 1));
    resize_main_page();
}
...
function display_product_infos(index) {/////index mean order for shipper
    ...
    for (var i = 0; i < orderList.length; i++) {
        if (orderList[i].id == parseInt(sessionStorage.getItem('cur_shipper_detail_index'))) {
            product_data += myShipperOrderDetailTemplate(orderList[i], index);
            break;
        }
    }
